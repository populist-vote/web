...
function HeaderSection({
  ...
  return (
    <section className={sectionCx}>
      ...
      />
      {!isMobile && <h1 className={headerStyles.fullName}>{politician?.fullName}</h1>}
      {false ? <Button id="add-note-btn" label={"Add note"} primary large theme="blue">
        Add note
      </Button> : (
        <div className={headerStyles.note}>
          ...
          <Button id="edit-note-btn" label={"Edit note"} secondary large theme="blue">Edit note</Button>
        </div>
      )}
    </section>
  );
