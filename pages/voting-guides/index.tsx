...
import { 
  ...
  VotingGuideResult } from "generated";
...
const getElectionsFromGuides = (guides: VotingGuideResult[]): VotingGuideElection[]=>{
  if (guides){
...
const VotingGuides: NextPage<{ mobileNavTitle?: string }> = ({mobileNavTitle}) => {
  ...
  if (isError){
    ...
    if (data && data.votingGuidesByUserId.length) {
      ...
      votingGuidesByUserId.refetch().then(res=>{
        const elections = getElectionsFromGuides(res.data?.votingGuidesByUserId as VotingGuideResult[]);
  ...
  return (
    <>
      ...
      <Layout mobileNavTitle={`${mobileNavTitle || 'Voting Guides'}`} showNavLogoOnMobile={false}>
        ...
        <div className={styles.roundedCard}>
          ...
          <input className={styles.guideInput} type="text" placeholder="Title" onChange={e=>setTitle(e.target.value)} value={title} />
          <input className={styles.guideInput} type="text" placeholder="Description" onChange={e=>setDescription(e.target.value)} value={description} />
          <Button large theme="blue" disabled={!title || !description} label="add_guide"
            onClick={()=>
